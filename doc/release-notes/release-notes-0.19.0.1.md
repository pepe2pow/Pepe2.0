...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  PEPE2), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in PEPE2 per 1,000
...
- #16521 Use the default maxfeerate value as PEPE2/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as PEPE2/kB (jonatack)
